...
module DataOutput
   ...
   integer(int32), private :: tau_int_unit = 151       ! File unit for tau_int
   integer(int32), private :: ess_unit = 152       ! File unit for ess
 ...
 contains
   ...
   subroutine InitializeHistoryOutput(filename)
    ...
    output_count=0
     ...
     allocate(history_buffer(history_length, BUFFER_SIZE))
     buffer_count = 0
     ! Write CSV header.
    ...
    open(unit = tau_int_unit, file = 'tau_int_history.txt', status='replace', action='write', iostat=iostat_local)
    open(unit = ess_unit, file = 'ess_history.txt', status='replace', action='write', iostat=iostat_local)
     write(tau_int_unit,fmt1) header(:)
     write(ess_unit,fmt1) header(:)
   ...
   subroutine FlushHistoryBuffer()
     use MCMCStats
     implicit none
     integer :: i,acf_size,ic
     !real(fp_kind) :: tau_int,ess
     real(fp_kind), allocatable,dimension(:) :: acf, tau_int, ess
    ...
    ic = 4+5+4*K1+4*K2
    allocate(tau_int(ic),ess(ic))
     do i=1,ic
       acf = ComputeAutocorrelation(history_buffer(i+1,:),acf_size)
       tau_int(i) = ComputeIntegratedAutocorrelationTime(acf)
       ess(i) = ComputeEffectiveSampleSize(history_buffer(i+1,:),tau_int(i))
     end do

    write(tau_int_unit,fmt2) real(output_count*BUFFER_SIZE,fp_kind),tau_int(:)
    write(    ess_unit,fmt2) real(output_count*BUFFER_SIZE,fp_kind),    ess(:)
